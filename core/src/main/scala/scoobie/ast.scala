...
object ast {
  ...
  case class QueryParameter[F[_], T](value: T, ev: F[T]) extends QueryValue[F]
  object QueryParameter {
    def apply[F[_], T](value: T)(implicit ev: F[T]): QueryParameter[F, T] = QueryParameter(value, ev)
  }

  case class QueryFunction[F[_]] private (path: QueryPath, args: List[QueryValue[F]]) extends QueryValue[F]
  ...
  class QueryNull[F[_]] extends QueryValue[F[_]]
  ...
  case class QueryAnd[F[_]](left: QueryValue[F], right: QueryValue[F]) extends QueryComparison[F]
  case class QueryOr[F[_]](left: QueryValue[F], right: QueryValue[F]) extends QueryComparison[F]
  ...
  case class QueryUpdate[F[_]] private (collection: QueryPath[F], values: List[ModifyField[_ <: HList]], where: QueryComparison[F]) extends QueryExpression[F] with QueryModify[F]
